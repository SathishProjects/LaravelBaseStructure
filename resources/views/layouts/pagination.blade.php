<div class="col-md-12 bottom" data-ng-if="records">
    <div class="col-md-4 default"> 
        <span>{{trans('messages.empty_record')}}</span> 
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    @else
                        <h4 class="modal-title"> {{trans('messages.delete_records')}} </h4>
                    @endif
                </div>
                <div class="modal-body">
                    <div data-ng-show="confirmationDeleteBox">
                        <p>{{trans('messages.delete_selected_records_confirmation')}}</p>
                    </div>
                </div>
                <div class="clearfix modal-footer">
                    <span data-ng-click="cancelDelete()" class="btn btn-danger pull-right"
                        data-dismiss="modal">{{trans('messages.cancel')}}</span>
                    <span data-ng-click="confirmDelete()" class="btn btn-primary pull-right mr10"
                        data-dismiss="modal">{{trans('messages.confirm')}}</span>
                 </div>
            </div>
        </div>
    </div>
